...
function NewsCard({ item }: { item: NewsItem }) {
  return (
    <article
      className={`relative bg-sf-mid cursor-pointer hover:bg-[#1a1a1e] transition-colors duration-200 group ${
        item.featured ? 'p-10' : 'p-8'
      }`}
    >
      <span className="inline-block text-[10px] font-bold tracking-[0.1em] uppercase bg-orange-500/15 text-sf-orange px-2.5 py-1 mb-4">
        {item.tag}
      </span>
      ...
      <p className="text-[13px] text-sf-muted leading-relaxed pr-10">
        {item.excerpt}
      </p>
  ...
  return (
    <div id="news" className="bg-sf-surface border-y border-sf-border">
      <div className="max-w-[1100px] mx-auto px-6 md:px-12 py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr] gap-[2px] mt-14">
